#!/usr/bin/env python3
"""Ingest Terceira events from Azores What's On (whatson.azores.gov.pt).

Source: https://whatson.azores.gov.pt/

The regional government's events portal aggregates listings across the whole
archipelago. We pull the EN listing page, extract JSON-LD `Event` blocks,
filter to events that mention "terceira" anywhere in their name, description,
venue, or address, and feed the rest through the shared dedup machinery.

If the page stops embedding JSON-LD (or starts requiring JS to render it),
this script will start no-opping; we'll switch to a different strategy
without breaking the cron.
...
SOURCE = JsonLdSource(
    slug="whatson.azores.gov.pt",
    listing_url="https://whatson.azores.gov.pt/en/",
    region_keyword="terceira",
    default_locality="Angra do Heroísmo",
)

logger = logging.getLogger("ingest_whatson_azores")
...
def main() -> int:
    ...
    logger.info("harvested %d JSON-LD Terceira events from %s", len(raw_events), SOURCE.listing_url)
    if not raw_events:
        return 0
    ...
    block = f"\n# Auto-ingested from Whatson Azores ({today.isoformat()})\n"
